       ...
       */
    	...
    	animateToPosition: function (_x, _y, _z, dur, _delay) {
    		TweenMax.to(instance, dur, {
    			...
    			onComplete: function () {
    				$('#gameView').css('webkitTransform', 'none'); //force re-render for svg preservation, but flickers when in ifame??
    				$('#gameView').css('transform', 'none'); //force re-render for svg preservation, but flickers when in ifame??
    			}
    		});
    	},
		...
		go: function(){
			...
			this.element.css('transformOrigin', this.x + 'px ' + this.y + 'px');
